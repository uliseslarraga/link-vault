...
# SSM access so Session Manager can connect to nodes without a bastion
...
# ── EBS CSI IRSA Role ─────────────────────────────────────────────────────────
# Scoped to the ebs-csi-controller-sa service account in kube-system.
# Allows the EBS CSI driver to call ec2:CreateVolume, ec2:AttachVolume, etc.
